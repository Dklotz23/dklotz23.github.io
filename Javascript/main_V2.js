...
function addSubLetter(loc) {
    ...
    if (!boardLoc.includes(loc)) { // Not in array
        boardLoc.push(loc);
        document.getElementById(loc).style.backgroundColor = 'Green'; // highlight green
    ...
    if (JSON.parse(localStorage.getItem('guessed_word_cnt')) < 5) {
        if (word != '') {
            ...
            if (word == word_four | word == word_five) {
                ...
                for (i=0; i < boardLoc.length; i++) {   
                    document.getElementById(boardLoc[i]).classList.add('rotate360'); // rotates letters guesses. (needs to be fixed for duplicate letters spinning...)
                ...
                setTimeout(remove_rotate,1000);   // need to move  
                boardLoc = [] // clear board
